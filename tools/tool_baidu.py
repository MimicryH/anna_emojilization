...
API_KEY = 'smoFcGrvVsPmYtLBGRqsVx9V'
SECRET_KEY = 'DsPLNzOzWEhqyKGTaGkh8w32PjaPePDv'
...
class Tool_Baidu():
    ...
    def asr(self, audio_file):
        _format = audio_file[-3:];
        with open(audio_file, 'rb') as speech_file:
            self.speech_data = speech_file.read()
        length = len(self.speech_data)
        if length == 0:
            raise DemoError('file %s length read 0 bytes' % AUDIO_FILE)
        speech = base64.b64encode(self.speech_data)
        if (IS_PY3):
        ...
        # print post_data
        print("waiting for response from BAIDU...")
...
if __name__ == '__main__':
    tool = Tool_Baidu()
    print(tool.asr('E:\DoobiePJ\doobieBot\SerDual\static\\test_audio\\1.wav'))
